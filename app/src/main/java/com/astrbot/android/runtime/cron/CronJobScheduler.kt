...
object CronJobScheduler {
    ...
    fun initialize(context: Context) {
        ...
        kotlinx.coroutines.CoroutineScope(Dispatchers.IO).launch {
            val enabledJobs = CronJobRepository.listEnabled()
            RuntimeLogRepository.append("CronJobScheduler: re-scheduling ${enabledJobs.size} enabled jobs")
    ...
    fun scheduleJob(context: Context, job: CronJob) {
        ...
        if (delayMs < 0) {
            RuntimeLogRepository.append("CronJobScheduler: skipping job ${job.jobId} — invalid delay")
            return
        }
        ...
        RuntimeLogRepository.append("CronJobScheduler: scheduled job ${job.jobId} '${job.name}' with delay ${delayMs}ms")
    }

    fun cancelJob(context: Context, jobId: String) {
        WorkManager.getInstance(context).cancelUniqueWork("cron_job_$jobId")
        RuntimeLogRepository.append("CronJobScheduler: cancelled job $jobId")
    }
