...
- version：`23`
- `exportSchema = true`
- schema：当前存在 `app/schemas/com.elymbot.android.data.db.ElymBotDatabase/23.json` 和 `core/db/schemas/com.elymbot.android.data.db.ElymBotDatabase/23.json`
- migrations：`astrBotDatabaseMigrations` 聚合 `migration2To3` 到 `migration22To23`
...
- `docs/archive/pre-uth-docs/docs-00-11/02_数据真源_Room_Repository_备份基线.md`：保留为 `core-foundation-and-db` 辅助证据；其中旧 Room 版本叙述已过期，当前 DB version 为 `23`，数据库入口应以当前 `core/db/src/main/java/com/elymbot/android/data/db/ElymBotDatabase.kt` 为准，不再使用旧 `app/src/main/java/.../ElymBotDatabase.kt` 路径。
