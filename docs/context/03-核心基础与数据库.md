...
- 下一模块：`core-runtime`
- 最近影响本模块的 scoped-sync：当前工作区 `D26060701` 将 Room schema 升级到 v24，并新增 geofence 规则、区域、Config 绑定和执行记录表。
- 本轮 Git 写入：无
...
- `core/db/src/main/java`：70 个 Kotlin/Java 文件
...
- `:core:db` 使用 Android library、AGP 9 built-in Kotlin、KSP、Room `2.8.4`、Room KTX 和 coroutines core。
...
- version：`24`
- `exportSchema = true`
- schema：当前存在 `app/schemas/com.elymbot.android.data.db.ElymBotDatabase/24.json` 和 `core/db/schemas/com.elymbot.android.data.db.ElymBotDatabase/24.json`
- migrations：`astrBotDatabaseMigrations` 聚合 `migration2To3` 到 `migration23To24`

当前 `ElymBotDatabase` 实体覆盖 app preferences、bot/config/conversation/persona/provider aggregate、plugin install/catalog/config snapshot/state、download tasks、saved QQ accounts、TTS voice assets、cron jobs/execution records、resource center items、config resource projections、geofence rules、geofence regions、config geofence bindings 和 geofence execution records。

`migration22To23` 为 config profile 增加 `pluginCommandsAdminOnlyEnabled`，该字段同时进入 `RuntimeConfigSnapshot`、backup JSON 映射和 legacy config import。当前语义用于控制插件 slash command 是否仅管理员可用。

`migration23To24` 新增 `geofence_rules`、`geofence_regions`、`config_geofence_bindings` 和 `geofence_execution_records`，并创建 rule/region、config binding 和 execution record 相关索引。`GeofenceRuleDao.deleteRule(...)` 当前按事务先删除 owned regions 与 config bindings，再删除 parent rule。
...
- 通过 Hilt 提供 `ElymBotDatabase` 和各 DAO；当前工作区新增 `provideGeofenceRuleDao(...)`。
...
- `docs/archive/pre-uth-docs/docs-00-11/02_数据真源_Room_Repository_备份基线.md`：保留为 `core-foundation-and-db` 辅助证据；其中旧 Room 版本叙述已过期，当前 DB version 为 `24`，数据库入口应以当前 `core/db/src/main/java/com/elymbot/android/data/db/ElymBotDatabase.kt` 为准，不再使用旧 `app/src/main/java/.../ElymBotDatabase.kt` 路径。
