# D26052102 剩余插件 API 能力索引

更新时间：2026-05-22 16:46 +08:00

## 用途

本文件只作为 `D26052102` 的剩余能力索引，不是新增 Todo、不是 worker prompt，也不是实现计划。

它用于告知当前完成情况，并列出后续仍可派发的剩余代办。当前 `T11` 与 `T12` 已完成实现、主控验证、evaluator 复核和 `uth-review` 验收复查；T06/T09 验收时记录的非阻塞审计字段增强也已收口，因此本索引不再保留 AstrBot 对齐剩余能力缺口。

## 当前完成情况

已完成并已有实现 / 验证记录：

| Scope | 状态 | 当前能力 |
| --- | --- | --- |
| `T01 宿主 API 异步桥与权限底座` | 已完成 | Host API facade、权限校验、审计、QuickJS await 基线 |
| `T02 宿主网络请求代理` | 已完成 | `hostApi.fetch` / `hostApi.network.request`，走宿主 `RuntimeNetworkTransport` |
| `T03 Provider 模型只读查询` | 已完成 | `hostApi.providers.list` / `hostApi.providers.models` |
| `T04 当前会话消息发送` | 已完成 | `hostApi.message.send`，当前会话约束 |
| `T05 当前会话历史只读查询` | 已完成 | `hostApi.conversation.history`，当前会话只读历史 |
| `T06 直接 LLM 调用` | 已验收 | `hostApi.callLlm` / `hostApi.llm.generate`，默认绕过插件 LLM hooks |
| `T07 上下文压缩落地` | 已验收 | `compress_context` 已接入宿主压缩实现，`hostApi.context.compress` 可用 |
| `T08 插件定时任务回调` | 已验收 | `hostApi.registerScheduledHandler`，Cron wakeup 进入 Plugin V2 handler |
| `T09 插件流式输出` | 已验收 | `hostApi.message.openStream`，App Chat editable 与 QQ final-on-close fallback |
| `T10 富消息链` | 已验收 | `PluginMessageSegment` 与 `hostApi.message.send({ chain })`，App Chat / QQ 映射与 fallback |
| `T11 Agent 能力注册` | 已验收 | `hostApi.registerAgent`、`hostApi.agent.run` 与 handler 内 `event.agent.run(input)`，受控 LLM/tool loop |
| `T12 Filter 组合表达式` | 已验收 | `allOf` / `anyOf` / `not` filter AST，旧 `declaredFilters` builtin-before-custom 兼容 |

T06 到 T10 的验收命令：

- `.\gradlew.bat :feature:plugin:runtime:testDebugUnitTest --tests "*Llm*" --tests "*Context*Compress*" --tests "*Scheduled*" --tests "*Stream*" --tests "*MessageSegment*" --tests "*RichMessage*" --tests "*PluginV2HostApiQuickJsCapabilitiesTest*" --console=plain --no-daemon --stacktrace`：pass，日志 `build/reports/D26052102-T06-T10-feature-plugin-runtime-tests.log`。
- `.\gradlew.bat :app:testDebugUnitTest --tests "*FutureToolSourceRegistryContextCompressionInjectionTest*" --tests "*ContextStrategyToolSourceProviderTest*" --tests "*CronJobRunCoordinatorTest*" --tests "*PluginV2RuntimeLoaderTest*" --tests "*PluginV2HostApiArchitectureContractTest*" --tests "*PluginV2BootstrapHostApiTest*" --tests "*PluginV2HostApiPermissionPolicyTest*" --tests "*PluginV2HostApiAuditLoggerTest*" --console=plain --no-daemon --stacktrace`：pass，日志 `build/reports/D26052102-T06-T10-app-tests.log`。
- `.\gradlew.bat :app-integration:testDebugUnitTest --tests "*PluginHostCapabilityModuleTest*" --console=plain --no-daemon --stacktrace`：pass，日志 `build/reports/D26052102-T06-T10-app-integration-tests.log`。
- `.\gradlew.bat architectureCheck --console=plain --no-daemon --stacktrace`：pass，日志 `build/reports/D26052102-T06-T10-architectureCheck.log`。
- `.\gradlew.bat modulePluginCheck --console=plain --no-daemon --stacktrace`：pass，日志 `build/reports/D26052102-T06-T10-modulePluginCheck.log`。
- `.\gradlew.bat clean assembleDebug --console=plain --no-daemon --stacktrace`：pass，日志 `build/reports/D26052102-T06-T10-clean-assembleDebug.log`。

上述 6 个日志的 `warning` / `deprecated` / `exception` / `failed` 扫描计数均为 0。

T11 到 T12 的实施与主控验证命令：

- `.\gradlew.bat :feature:plugin:runtime:testDebugUnitTest --tests '*Agent*' --tests '*Filter*' --tests '*PluginV2HostApiQuickJsCapabilitiesTest*' --console=plain --no-daemon --stacktrace`：pass，日志 `build/reports/D26052102-T11-T12-feature-plugin-runtime-tests.log`。
- `.\gradlew.bat :app:testDebugUnitTest --tests '*PluginV2*' --tests '*Plugin*' --tests '*ToolSource*' --console=plain --no-daemon --stacktrace`：pass，日志 `build/reports/D26052102-T11-T12-app-tests.log`。
- `.\gradlew.bat architectureCheck --console=plain --no-daemon --stacktrace`：pass，日志 `build/reports/D26052102-T11-T12-architectureCheck.log`。
- `.\gradlew.bat modulePluginCheck --console=plain --no-daemon --stacktrace`：pass，日志 `build/reports/D26052102-T11-T12-modulePluginCheck.log`。
- `.\gradlew.bat clean assembleDebug --console=plain --no-daemon --stacktrace`：pass，日志 `build/reports/D26052102-T11-T12-clean-assembleDebug.log`。

上述 5 个日志的 `warning` / `deprecated` / `exception` / `failed` 扫描计数均为 0。

T11 到 T12 的验收复查命令：

- `.\gradlew.bat :feature:plugin:runtime:testDebugUnitTest --tests "*Agent*" --tests "*Filter*" --tests "*PluginV2HostApiQuickJsCapabilitiesTest*" --console=plain --no-daemon --stacktrace`：pass，日志 `build/reports/D26052102-T11-T12-review-feature-plugin-runtime-tests.log`。
- `.\gradlew.bat :app:testDebugUnitTest --tests "*PluginV2*" --tests "*Plugin*" --tests "*ToolSource*" --console=plain --no-daemon --stacktrace`：pass，日志 `build/reports/D26052102-T11-T12-review-app-tests.log`。
- `.\gradlew.bat architectureCheck --console=plain --no-daemon --stacktrace`：pass，日志 `build/reports/D26052102-T11-T12-review-architectureCheck.log`。
- `.\gradlew.bat modulePluginCheck --console=plain --no-daemon --stacktrace`：pass，日志 `build/reports/D26052102-T11-T12-review-modulePluginCheck.log`。
- `.\gradlew.bat clean assembleDebug --console=plain --no-daemon --stacktrace`：pass，日志 `build/reports/D26052102-T11-T12-review-clean-assembleDebug.log`。

上述 5 个验收复查日志的 `warning` / `deprecated` / `exception` / `failed` 扫描计数均为 0。
非阻塞审计字段收口命令：

- `.\gradlew.bat :feature:plugin:runtime:testDebugUnitTest --tests "*PluginV2HostLlmApiTest*" --tests "*PluginV2MessageStreamApiTest*" --tests "*PluginV2HostApiAuditLoggerTest*" --console=plain --no-daemon --stacktrace`：pass。
- `.\gradlew.bat :app:testDebugUnitTest --tests "*PluginV2HostApiAuditLoggerTest*" --console=plain --no-daemon --stacktrace`：pass。
- `.\gradlew.bat architectureCheck --console=plain --no-daemon --stacktrace`：pass。
- `.\gradlew.bat modulePluginCheck --console=plain --no-daemon --stacktrace`：pass。
- `.\gradlew.bat clean assembleDebug --console=plain --no-daemon --stacktrace`：pass，日志 `build/reports/D26052102-nonblocking-audit-fields-clean-assembleDebug.log`。

`build/reports/D26052102-nonblocking-audit-fields-clean-assembleDebug.log` 的 `warning` / `deprecated` / `exception` / `failed` 扫描计数均为 0。

已有基础能力，不作为本轮剩余缺口：

- message / command / regex handler 注册。
- lifecycle / LLM hook / tool / tool lifecycle 注册。
- plugin/session storage。
- settings / metadata / log。
- ToolSource 聚合与 host builtin tool 基础。

## 剩余代办

| Order | Todo | 状态 | 说明 |
| --- | --- | --- | --- |
| - | - | 无 | `D26052102` 当前设计内 `T01` 到 `T12` 均已有实现 / 验证 / 复核 / 验收记录 |

## 验收备注

- T06/T09 当前通过功能、权限、结构与构建验收；LLM provider/model/token usage 与 streamId/chunk/bytes/duration 专用审计字段已在本轮非阻塞收口中落地。该项不再作为 AstrBot 对齐能力缺口或非阻塞残留保留。
- T11 已在 T06、T03 与现有 ToolSource / tool registry 基础上完成并通过本轮 `uth-review` 验收复查；T12 已完成旧 `declaredFilters` 兼容与新 AST 表达力并通过本轮 `uth-review` 验收复查。
- 后续如需整包关闭或合并本任务包，可进入 Design-level 验收，再按用户确认进入 `uth-git`。

## 不做项

继续保持用户已确认边界：

- 不做 Web API 注册。
- 不做平台适配器注册。
- 不做 DB 直连。
- 不做文本转图。
- 不做 HTML 渲染。
- 不暴露 AstrBot 风格别名。

## 后续路由

- 当前无剩余非验证类 Todo 待派发。
- 如需关闭 `D26052102` 整包，可进入 Design-level 验收；如需提交，则按用户确认进入 `uth-git`。
- `Needs uth-docs scoped-sync`：插件平台上下文 `docs/context/09-插件平台.md` 需要在验收后同步 T01-T12 当前代码事实。
