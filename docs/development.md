...
- Room：`app/build.gradle.kts` 使用 `2.8.4`，`core/db/.../ElymBotDatabase.kt` 当前 DB version 为 `23`。
- App 版本：`app/build.gradle.kts` 当前为 `versionName = "1.1.0"`、`versionCode = 81`。
...
- `a9aace9^..HEAD` 范围后，`v0.9.3` 已完成 AGP 9 构建链升级，`v1.0.0` 已完成 ElymBot 包名与应用标识统一；当前 `HEAD` 为 `13467db ci: refresh validation workflow`。本地已发现 `v1.0.1`、`v1.0.2`、`v1.0.3`、`v1.1.0` tag；未发现 `v1.0.4` tag。
